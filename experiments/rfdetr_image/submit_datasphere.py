"""Render and submit the RF-DETR Datasphere job without storing secrets."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        rendered = (
            ...
            .replace("__IMAGE_DATASET_IMAGES_DIR__", paths["images_dir"])
            .replace("__IMAGE_DATASET_ANNOTATIONS_DIR__", paths["annotations_dir"])
            .replace("__RFDETR_OUTPUT_ROOT__", paths["output_root"])
        )
...
def _build_project_archive(project_root: Path, archive_path: Path) -> None:
    """Archive only the runtime source tree; secrets stay outside the job."""
