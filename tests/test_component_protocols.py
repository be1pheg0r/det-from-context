...
from context_detection.experiment import ExperimentProtocol, ExperimentRun
...
def test_experiment_dataset_and_model_protocols_work_together(tmp_path: Path) -> None:
    ...
    def worker(run: ExperimentRun, config: ExperimentConfig) -> dict[str, Any]:
        loader: DataLoader[Any] = build_dataset(config, "train")
        model: nn.Module = build_model(config)
        batch, context = next(iter(loader))
    ...
    result_root: Path = ExperimentProtocol(tmp_path).execute(config_path, worker)
