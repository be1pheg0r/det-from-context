...
Одна команда вместо пяти скриптов:
    python tools/cli.py train    configs/memory_ema.yaml
    python tools/cli.py eval     configs/memory_ema.yaml --ckpt ...
    python tools/cli.py smoke    configs/baseline.yaml
    python tools/cli.py showbatch configs/baseline.yaml
    python tools/cli.py profile  configs/memory_stream.yaml
    python tools/cli.py grid     configs/*.yaml        # сетка режимов отказа
"""
