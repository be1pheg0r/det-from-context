...
class MultiScaleProjector(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int, n_levels: int = 3):
        ...
        self.input_proj = nn.Sequential(
            nn.Conv2d(in_dim, hidden_dim, 1), nn.GroupNorm(32, hidden_dim)
        )
...
class DINOv3Backbone(nn.Module):
    def __init__(
        self,
        hf_name: str = "facebook/dinov3-vits16-pretrain-lvd1689m",
        freeze: bool = False,
    ):
        super().__init__()
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        return (
            tokens[:, self.num_special_tokens :, :]
            .transpose(1, 2)
...
class MSDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=3, n_heads=8, n_points=4):
        ...
        self.value_proj, self.output_proj = (
            nn.Linear(d_model, d_model),
            nn.Linear(d_model, d_model),
        )
        for p in [self.sampling_offsets, self.attention_weights]:
    ...
    def forward(self, query, reference_points, value, spatial_shapes):
        ...
        attn = F.softmax(
            self.attention_weights(query).view(B, Q, H, L * P), dim=-1
        ).view(B, Q, H, L, P)

        out = query.new_zeros(B, H, D, Q)
        for lvl, (h, w) in enumerate(spatial_shapes):
            v = value_list[lvl].permute(0, 2, 3, 1).reshape(B * H, D, h, w)
            loc = reference_points[:, :, None, None, :] + offsets[
                :, :, :, lvl
            ] / query.new_tensor([w, h])
            loc = loc.clamp(0.0, 1.0) * 2 - 1
            sampled = F.grid_sample(
                v,
                loc.permute(0, 2, 1, 3, 4).reshape(B * H, Q, P, 2),
                mode="bilinear",
                align_corners=False,
            )
            w_lvl = attn[:, :, :, lvl, :].permute(0, 2, 1, 3)
            out = out + torch.einsum(
                "bhdqp,bhqp->bhdq", sampled.view(B, H, D, Q, P), w_lvl
            )
...
class DeformableDecoder(nn.Module):
    def __init__(self, d_model=256, num_layers=6, num_classes=10):
        ...
        self.layers = nn.ModuleList(
            [DeformableDecoderLayer(d_model) for _ in range(num_layers)]
        )
    ...
    def forward(self, tgt, query_pos, ref, memory, spatial_shapes):
        ...
        for i, layer in enumerate(self.layers):
            ...
            box = (
                torch.cat([inverse_sigmoid(ref), torch.zeros_like(ref)], dim=-1) + delta
            ).sigmoid()
...
class RFDetrAdapter(DetectorAdapter):
    def __init__(self, variant: str = "base", weights: str | None = None):
        super().__init__()
        self.hidden_dim, self.num_classes, self.num_queries, self.n_levels = (
            256,
            10,
            300,
            3,
        ...
        self.enc_output, self.enc_output_norm = (
    ...
    def initial_queries(self, batch: DetectionBatch) -> torch.Tensor:
        ...
        return torch.zeros(
            b, self.num_queries, self.hidden_dim, device=batch.images.device
        )
    ...
    def forward(
        ...
        for H, W in shapes:
            gy, gx = torch.meshgrid(
                (torch.arange(H, device=memory.device) + 0.5) / H,
                (torch.arange(W, device=memory.device) + 0.5) / W,
                indexing="ij",
            )
            anchors.append(
                torch.cat(
                    [
                        torch.stack([gx, gy], -1).reshape(-1, 2),
                        torch.full((H * W, 2), 1.0 / max(H, W), device=memory.device),
                    ],
                    -1,
                )
            )
        ...
        tgt, ref_points = (
            torch.gather(enc, 1, idx_c).detach(),
            torch.gather(enc_boxes, 1, idx.unsqueeze(-1).expand(-1, -1, 4))[
                ..., :2
            ].detach(),
        )
        ...
        logits, boxes = self.decoder(
            tgt,
            self.query_pos_mlp(torch.gather(pos_flat, 1, idx_c)),
            ref_points,
            memory,
            shapes,
        )
